...
export async function generateMetadata({
  ...
  if (blank) {
    ...
    const title = blank.metaTitle;
    const description = blank.metaDescription;
    return {
...
export default async function ServiceDetailPage({
  ...
  if (blank)
    return (
      <>
        <ServiceLanding content={blank} />
        <Footer />
      </>
    );
  const t = getT("serviceDetail");
  const service = getServiceBySlug(slug);
  if (!service) notFound();
  const url = `https://rizon.agency/services/${service.slug}`;
  const serviceTitle = service.title;
  const targetKeyword = service.targetKeyword;
        ...
        <section className="container cntr mt-24 md:mt-32">
          <div className="surface px-6 py-10 md:px-10 md:py-14">
            ...
            {service.stat && (
              <div className="mt-10">
                <StatBlock
                  stat={{ ...service.stat, label: service.stat.label }}
                />
              </div>
            )}
          </div>
